...
{
    public partial class NgayNghiNam : BackEndPageBase
    {
        ...
        void LoadData()
        {
            ...
            Store1.DataSource = db.tbNgayNghiPhepNams.Select(i => new
            {
                ngay = i.ngay + "/" + i.thang + ((i.nam == null || i.nam <= 0) ? "" : ("/" + i.nam)),
                ten = i.ten,
                id = i.id
            });
            Store1.DataBind();
        ...
        {
            var db = new global::iHRM.Core.Business.DbObject.dcDatabaseDataContext(global::iHRM.Core.Business.Provider.ConnectionString);
            var it = db.tbNgayNghiPhepNams.SingleOrDefault(i => i.id == int.Parse(commandId));
        ...
        private void DeleteRecord(string commandId)
        {
            ...
            if (it != null)
            {
                db.tbNgayNghiPhepNams.DeleteOnSubmit(it);
                db.SubmitChanges();
            }
        }
        ...
        protected void btnDangKy_DirectClick(object sender, DirectEventArgs e)
        {
            ...
            {
                tbNgayNghiPhepNam it = new tbNgayNghiPhepNam();
                FormGetDataContext(frmEditor, it);
                db.tbNgayNghiPhepNams.InsertOnSubmit(it);
                db.SubmitChanges();
                Tools.message(Lng.common_msg.Add_Success);
            }
            else
            {
                var it = db.tbNgayNghiPhepNams.SingleOrDefault(i => i.id == int.Parse(h_id.Value as string));
                FormGetDataContext(frmEditor, it);
                try
                {
                    db.SubmitChanges();
