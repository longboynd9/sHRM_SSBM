...
{
    public partial class rptOT6H : BackEndPageBase
    {
        ...
        protected void Search(object sender, DirectEventArgs e)
        {
            ...
            if (txtDate.SelectedDate != null && todate.SelectedDate != null)
            {
                ...
                data = logic.GetReportTangCa6H(day, todates, txtma.Text != "" ? txtma.Text : null, DepID);
                if(data!=null)
        ...
        protected void btnExcel_DirectClick(object sender, DirectEventArgs e)
        {
            ...
            if (txtDate.SelectedDate != null && todate.SelectedDate != null)
            {
                ...
                if (data != null)
                {
                    if (data.Rows.Count > 0)
                    {
                        xuatexcel(data, "DanhsachOT.xlsx", 7, 1);
                    }
                }
                        ...
                        using (ExcelPackage xlPackage = new ExcelPackage(fileTemp, true))
                        {
                            ...
                            try
                            {
                                ...
                                foreach (DataRow _dr in dsData.Rows)
                                {
                                    ...
                                    worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 2].Value = _dr["EmployeeName"].ToString();
                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 3].Value = Convert.ToDateTime(_dr["Birthday"]);
                                    }
                                    catch
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 3].Value = _dr["Birthday"].ToString();
                                    }
                                    worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 4].Value = _dr["IDCard"].ToString();
                                    worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 5].Value = _dr["CardID"].ToString();
                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 6].Value = _dr["PosName"].ToString();
                                    }
                                    catch (Exception)
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 6].Value = "";
                                    }
                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 7].Value = Convert.ToDateTime(_dr["AppliedDate"]);
                                    }
                                    catch (Exception)
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 7].Value = _dr["AppliedDate"].ToString();
                                    }
                                    worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 8].Value = _dr["ngay"].ToString();
                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 9].Value = Math.Round(Convert.ToDouble(_dr["tgTinhTangCa"].ToString()), 2);
                                    }
                                    catch (Exception)
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 9].Value = "";
                                    }

                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 10].Value = TimeSpan.Parse(_dr["tgQuetDen"].ToString());
                                    }
                                    catch (Exception)
                                    {
                                    ...
                                    try
                                    {
                                        worksheet.Cells[iStartRow + _irow, iStartCol + _indexcol + 11].Value = TimeSpan.Parse(_dr["tgQuetVe"].ToString());
                                    }
                                    catch (Exception)
                                    {
