                ...
                foreach (string eID in lstEmp)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var kq in kqs)
                        {
                            ...
                            if (kq.tt_leTet != 1)
                            {
                                if (kq.ngay.DayOfWeek != DayOfWeek.Sunday)
                                {
                                    if (!kq.Istt_nghiPhep_AliasNull() && kq.tt_nghiPhep_Alias == "KP")
                                    {
                                        CountnghiKhongPhep++;
                                    }
                                    // Nếu có quẹt sáng hoặc chiều, nghỉ mà k xin phép. K đc tính chuyên cần.
                                    if ((kq.Istt_nghiPhep_AliasNull() || (!kq.Istt_nghiPhep_AliasNull() && kq.tt_nghiPhep_Alias == "")) && (kq.tt_coQuetTay != 3))
                                    {
                                        CountnghiKhongPhep++;
                                    }
                                }
                            ...
                            if (kq.tuGio >= new TimeSpan(12, 0, 0) && kq.denGio == new TimeSpan(8, 0, 0) && !kq.IskqNgayCongNull() && kq.kqNgayCong > 0 && !kq.caDem && kq.idCaLam.ToString() != "e630553b-f7c0-4089-95b6-4ca9a0bf9a71")
                            {
                                r.thuongLamCa += 20000;
                            }
                            #endregion
                            #region tinh thưởng bữa tối theo lương mới.(ca Hành chính SX: 7h30 - 16h30 )
                            r.thuongBuaToi += hp.TinhThuongBuaToi();
                            #endregion
                            ...
                            if (hp.luongCu != null)
                            {
                                ...
                                r.tienTangCa_lt_Cu += hp.TinhTienTangCa(3, true);
                                ...
                                r.thuongBuaToi += hp.TinhThuongBuaToi(true);
                                #endregion
                            }
                        }
                        #endregion
                        ...
                        r.tongTgTangCa_Moi = r.tgTangCa_bt_Moi + r.tgTangCa_Dem_Moi + (r.ngayCong_cn_Moi * 8) + r.tgTangCa_cn_Moi + r.tgTangCa_lt_Moi;
                        r.tongTgTangCa_Cu = r.tgTangCa_bt_Cu + r.tgTangCa_Dem_Cu + (r.ngayCong_cn_Cu * 8) + r.tgTangCa_cn_Cu + r.tgTangCa_lt_Cu;
                        ...
                        // Kiểm tra xem có bị trừ chuyên cần không.
                        var e = db.tblEmployees.Where(p => p.EmployeeID == eID).FirstOrDefault();
                        if (!(e != null && e.isTinhChuyenCan == true))
                        {
                            r.tongThuongCalc = r.tongThuongCalc - (double)r["Calc1"];
                            r["Calc1"] = 0;
                        }
                        else if (CountnghiKhongPhep >= 1)// Nếu nghỉ không phép -> Tiền chuyên cần = 0.
                        {
                            r.tongThuongCalc = r.tongThuongCalc - (double)r["Calc1"];
                            r["Calc1"] = 0;
                        }
                        ...
                        {
                            r.isLuongSP = true;
                            // Nếu lương sản phẩm cao hơn. Không cộng với PC3: khoản thưởng khác, thưởng bữa tối, PC4: Khoản khác. 
                            r.tongLuong = r.tongLuongSP - r.thuongBuaToi - (double)r["PC3"] - (double)r["PC4"];
                        }
                        else
                        {
                            r.isLuongSP = false;
                            r.tongLuong = r.tongLuongTG;
                        }
                        ...
                        if (e1["coBH"] != DBNull.Value && e1.coBH)
                        {
                            ...
                            {
                                r.tienBHXH = r.luongCB_Moi * 8 / 100;
                                r.tienBHYT = r.luongCB_Moi * 1.5 / 100;
                                r.tienBHTN = r.luongCB_Moi * 1 / 100;
                                r.tienBH105 = r.tienBHXH + r.tienBHYT + r.tienBHTN;
                                r.phiCongDoan = r.luongCB_Moi / 100;
                            }
                        }
