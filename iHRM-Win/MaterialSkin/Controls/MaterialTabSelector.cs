    ...
    public class MaterialTabSelector : Control, IMaterialControl
    {
        ...
        private void UpdateTabRects()
        {
            ...
            using (var b = new Bitmap(1, 1))
            {
                ...
                {
                    tabRects.Add(new Rectangle(SkinManager.FORM_PADDING, 0, TAB_HEADER_PADDING * 2 + (int)g.MeasureString(baseTabControl.TabPages[0].Text, SkinManager.Font_SMALL).Width, Height));
                    for (int i = 1; i < baseTabControl.TabPages.Count; i++)
                    {
                        tabRects.Add(new Rectangle(tabRects[i - 1].Right, 0, TAB_HEADER_PADDING * 2 + (int)g.MeasureString(baseTabControl.TabPages[i].Text, SkinManager.Font_SMALL).Width, Height));
                    }
                }
            }
        }
